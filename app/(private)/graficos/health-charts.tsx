...
export function HealthCharts() {
    ...
    series: [
      {
        ...
        data: data.moodTimeline.map((m) => ({
          ...
          itemStyle: {
            color: moodColorByCategory[m.mood] ?? colors.muted,
          },
        })),
      },
    ],
    ...
    series: [
      {
        ...
        label: {
          ...
          formatter: (p: any) =>
            `${p.value}/${data.indicators[p.dataIndex].max}`,
    ...
    series: [
      {
        ...
        data: data.flowLevels.map((f) => ({
          ...
          itemStyle: {
            ...
            color:
              flowColorByLabel[f.label] ?? colors.wine,
          },
        })),
  ...
  const maxIntensity = Math.max(
    1,
    ...data.calendarData.map(([, value]) => value)
  );
  ...
  return (
    <div className={styles.echartsGrid}>
      {charts.map(
        ([title, period, option, size], index) => (
          <article
            className={`${styles.echartCard} ${styles[size]}`}
            key={title}
          >
            <header>
              <div>
